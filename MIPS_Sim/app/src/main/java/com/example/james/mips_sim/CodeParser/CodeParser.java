...
public class CodeParser {
    ...
    //*****************************************************
    //parseLabels:
    // Identifies labels from assembly file and
    // sets a hashmap with <Label,memIndex>.
    // Ex: loop2, 54 -- Where loop2 is the label and 54 is the instruction index.
    //*****************************************************
    public static void parseLabels(String[] unParsedCode)
    {

        int memIndex = 1;
        for(int i = 0; i<unParsedCode.length; i++)
        {
            String lineInstruction = unParsedCode[i];
    ...
    public Queue<Long> parseCode(String[] unParsedCode)
    {
        // Create the HashMap of labels and their indexes... example: j, 54
        parseLabels(unParsedCode);

        for(int i = 0; i<unParsedCode.length; i++){
            Log.d("BENNNY", unParsedCode[i]);
        }
...
//            {
//                lineInstruction = lineInstruction.trim();
//            }
            ...
            System.out.println("Contains " + opString + " " + hashMap_op.containsKey(opString));
            ...
            else if (opcode == 2 || opcode == 3)
            {
                ...
                System.out.println(hashMap_loops == null);
...
//                tempInstruct_I[2] = getRegFromStr(split[1]);
                ...
                //  Gets the values from the parameters passed in

                // Makes an int array [op][rs][rt][immediate]
                switch (opString){
                case "lw":
                case "sw":
                    tempInstruct_I[0] = opcode;
                ...
                // For example: -4 --> 1111111111111111111111100 needs to just be 16 bits long
                // if it is +4, it needs bits appended. 001 --> 16 bits long
    ...
    public Instruction parseInstruction (long intNUM)
    {
        ...
        else if (opcode == 1 || opcode > 3 && opcode < 63)
        {
            ...
            System.out.println("Printing out I-Type:");
            for (int i = 0; i < intArray.length; i++)
            {
    ...
    public static String[] getCode(EditText code){

        return code.getText().toString().split("\n");
